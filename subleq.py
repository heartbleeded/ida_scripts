...
class Subleq(Architecture):
    ...
    address_size = 2
    default_int_size = 2
    max_instr_length = 3 * address_size # Each instruction is 3 words
    ...
    def perform_get_instruction_info(self,data,addr):
        ...
        if c != 0:
            if b == a:
                ...
                res.add_branch(BranchType.UnconditionalBranch, c*2)
            else:
                # True branch jumps to integer index c 
                res.add_branch(BranchType.TrueBranch, c*2)
    ...
    def perform_get_instruction_text(self, data, addr):
        ...
        # Unpack our operands from the data
        a,b,c = struct.unpack('<3H',data[:2*3])
        ...
        # clear [B]
            ...
            tokens.append(makeToken('a', hexr(b*2), b*2))
            tokens.append(makeToken('t', ']'))
        ...
        # sub [B], [A]
            ...
            tokens.append(makeToken('t', '['))
            tokens.append(makeToken('a', hexr(a*2), a*2))
            tokens.append(makeToken('t', ']'))
        ...
        if c != 0 and b == a:
            ...
            tokens.append(makeToken('a', hex(c*2), c*2))
        ...
        # ; jmp C if [B] <= 0
            ...
            tokens.append(makeToken('t', '['))
            tokens.append(makeToken('a', hex(b*2), b*2))
            tokens.append(makeToken('t', ']'))
    ...
    def perform_get_instruction_low_level_il(self, data, addr, il):
        ...
        addr_a = il.const_pointer(2, a*2)
        addr_b = il.const_pointer(2, b*2)
        addr_c = il.const_pointer(2, c*2)

        # mem[A] and mem[B] pointers
        mem_a = il.load(2, addr_a)
        mem_b = il.load(2, addr_b)

        # For a clear instruction just store 0
        if a == b:
            # *B = 0
            store_b = il.store(2, addr_b, il.const(2,0))
            il.append(store_b)

        # For normal operation, construct a subtraction
        else:
            # *B = *B - *A
            sub_op = il.sub(2, mem_b, mem_a)
            store_b = il.store(2, addr_b, sub_op)
            il.append(store_b)
        ...
        # Conditional jump
            ...
            t_target = il.get_label_for_address(Architecture['subleq'],
                    il[il.const_pointer(2, c*2)].constant)
            ...
            less_op = il.compare_signed_less_equal(2, mem_b, il.const(2, 0))
