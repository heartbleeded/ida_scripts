...
class pwn_Plugin_t(idaapi.plugin_t):
    wanted_name = "pwn"
    ...
    def patcher(self):
        if self.first_run==0:
            ...
            self.first_run=1
            return
        if idc.load_debugger('linux',1):
            pid = open('/tmp/pid','rb').read()
            idc.attach_process(int(pid.strip()), -1)
            time.sleep(0.5)
